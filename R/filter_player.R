...
#' \code{\link{per_player_agg}}, and inserts default values for any filters
...
#'   function \code{\link{per_player_agg}} with argument \code{clutch=FALSE}.
#'
#' @seealso \code{\link{per_player_agg}}
#' @export
filter_per_player <- function(...) {

    if ('measurement' %in% names(list(...))) {
        stop(paste('[statsnbaR filter_per_player] cannot specify',
                   '\'measurement\' as an argument to the filter constructor'))
    }
    if ('clutch' %in% names(list(...))) {
        stop(paste('[statsnbaR filter_per_player] cannot specify',
                   '\'clutch\' as an argument to the filter constructor'))
    }

    filter_construction_worker(statsnbaR.ADL.endpoints$per_player_base,
                               ...)
...
#' @description
#' Creates a list of key-value pairs from the supplied arguments for use in
#' \code{\link{per_player_agg}} with argument \code{clutch=TRUE}; and inserts
...
#'
#' @return A list of key-value pairs for passing to the per-player data query
#'   function \code{\link{per_player_agg}} with argument \code{clutch=TRUE}.
#'
#' @seealso \code{\link{per_player_agg}} \code{\link{filter_per_player}}
#' @export
filter_per_player_clutch <- function(...) {
    if ('measurement' %in% names(list(...))) {
        stop(paste('[statsnbaR filter_per_player_clutch] cannot specify',
                   '\'measurement\' as an argument to the filter constructor'))
    }
    if ('clutch' %in% names(list(...))) {
        stop(paste('[statsnbaR filter_per_player_clutch] cannot specify',
    ...
    filter_construction_worker(statsnbaR.ADL.endpoints$per_player_base_clutch,
                               ...)

}

#' Generate filters for player biography data query.
#'
#' @description
#' Creates a list of key-value pairs from the supplied arguments for use in
#' \code{\link{get_bio}}, and inserts default values for any filters
#' not specified in the arguments.
#'
#' @details
#' Returns a named list of key-value pairs which are given either by the
...
#'
#' @param ... List of key-value pairs to use as filters in a query to the
#'   player biography endpoint of stats.nba.com. The recognised filters are
#'   described here with their default values indicated in brackets:
#' \describe{
#'     \item{college}{character - full name of college of player (all)}
#'     \item{conference}{character - conference of player: 'both', 'west',
...
#' }
#'
#' @return A list of key-value pairs for passing to the player biography data
#'   query function \code{\link{get_bio}}.
#'
#' @seealso \code{\link{get_bio}}
#' @export
filter_bio <- function(...) {

    filter_construction_worker(statsnbaR.ADL.endpoints$player_bio,
                               ...)
